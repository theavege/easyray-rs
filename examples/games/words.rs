        ...
        fn handle(&mut self, event: Event) {
            match event {
                ...
                Event::Key(value) => match (&self.page, value.code, value.kind) {
                    (&Page::Welcome, KeyCode::Esc, KeyEventKind::Press) => self.npc.clear(),
                    (&Page::Welcome, KeyCode::Tab, KeyEventKind::Press) => {
                        self.store.lang = self.store.lang.switch();
                    }
                    (&Page::Welcome, KeyCode::F(10), KeyEventKind::Press) => {
                        self.store.level = self.store.level.switch();
                    }
                    (&Page::Welcome, KeyCode::Enter, KeyEventKind::Press) => {
                        self.page = Page::Main;
                        self.avatar.clear();
                        self.append();
                    }
                    (&Page::Main, KeyCode::Char(value), KeyEventKind::Press) => {
                        if !self.letter(value) {
                            self.page = Page::Welcome;
                        }
                    }
                    (&Page::Main, KeyCode::Esc, KeyEventKind::Press) => {
                        self.page = Page::Welcome;
                    }
                    _ => {},
                },
        ...
        fn draw(&self, frame: &mut Frame) {
            match self.page {
                Page::Welcome => {
                    ...
                    let [title, body] = Layout::vertical([Constraint::Ratio(1, 3); 2])
